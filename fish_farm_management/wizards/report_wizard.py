...
class FishFarmReportWizard(models.TransientModel):
    ...
    # فلاتر صحة الأسماك وجودة المياه
    ...
    def _prepare_report_filters(self):
        self.ensure_one()
        return {
            'date_from': self.date_from and self.date_from.strftime('%Y-%m-%d'),
            'date_to':   self.date_to   and self.date_to.strftime('%Y-%m-%d'),
    ...
    def action_print_report_pdf(self):
        self.ensure_one()
        provider = self.env['fish_farm_management.report_data_provider']
        ...
        if self.report_type == 'cost_analysis':
            ...
            report_ref = 'fish_farm_management.action_report_pond_cost_analysis'
        elif self.report_type == 'harvest_performance':
            data = provider.get_harvest_performance_report_data(filters)
            report_ref = 'fish_farm_management.action_report_harvest_performance'
        elif self.report_type == 'supplies_consumption':
            data = provider.get_supplies_consumption_report_data(filters)
            report_ref = 'fish_farm_management.action_report_supplies_consumption'
        elif self.report_type == 'fish_health_water_quality':
            data = provider.get_fish_health_water_quality_report_data(filters)
            report_ref = 'fish_farm_management.action_report_fish_health_water_quality'
        elif self.report_type == 'sales_profitability':
            data = provider.get_sales_profitability_report_data(filters)
            report_ref = 'fish_farm_management.action_report_sales_profitability'
        else:
            raise UserError(_("نوع التقرير غير مدعوم."))

        # إضافة معايير العرض إلى البيانات
        data['filters_display'] = {
            ...
            'issue_type':       dict(self.fields['issue_type'].selection).get(self.issue_type, ''),
            'customer':         self.customer_id.name,
            'sales_channel':    dict(self.fields['sales_channel'].selection).get(self.sales_channel, ''),
        }
        data['company_name']   = self.env.company.name
        data['company_logo']   = self.env.company.logo.decode('utf-8') if self.env.company.logo else False
