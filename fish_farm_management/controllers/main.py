...
class FishFarmReportController(http.Controller):
    ...
    def get_dashboard_data(self):
        ...
        active_ponds_count = request.env["fish_farm_management.pond"].search_count(
            [("status", "=", "stocked"), ("company_id", "=", company_id)]
        )
        ...
        harvest_recs = request.env["fish_farm_management.harvest_record"].search(
            [
        ...
        # Costs this month
        cost_recs = request.env["fish_farm_management.pond_cost"].search(
            [
        ...
        feed_recs = request.env["fish_farm_management.pond_feeding"].search(
            [
    ...
    def _get_costs_by_type_data(self, company_id):
        ...
        for i in range(6):
            month_start = date_utils.subtract(
                date_utils.start_of(current_date, "month"), months=i
            )
            ...
            costs = request.env["fish_farm_management.pond_cost"].read_group(
                ...
                orderby="cost_type_id",
            )

            labels.append(month_start.strftime("%b %Y"))
            month_costs = {
                (grp["cost_type_id"][1] if grp["cost_type_id"] else _("غير محدد")): grp[
                    "amount"
                ]
                for grp in costs
            }
        ...
        datasets = [
            {"label": ct, "data": [month.get(ct, 0.0) for month in data]}
    ...
    def _get_harvest_by_month_data(self, company_id):
        ...
        for i in range(12):
            ...
            month_end = date_utils.end_of(month_start, "month")
            recs = request.env["fish_farm_management.harvest_record"].search(
                [
    ...
    def _get_pond_status_data(self, company_id):
        ...
        status_counts = request.env["fish_farm_management.pond"].read_group(
            [("company_id", "=", company_id)], fields=["status"], groupby=["status"]
        )
        ...
        for group in status_counts:
            code = group["status"]
            name = dict(
                request.env["fish_farm_management.pond"]._fields["status"].selection
            ).get(code, code)
            labels.append(name)
        ...
        return {
            "labels": labels,
            "datasets": [
                {"data": data, "backgroundColor": bg_colors, "hoverOffset": 4}
            ],
        }
