...
class PondCost(models.Model):
    ...
    # الربط بالحساب التحليلي مباشرة
    analytic_account_id = fields.Many2one('account.analytic.account', string='الحساب التحليلي', required=True, ondelete='restrict',
                                         help='الحساب التحليلي لتصنيف التكلفة (مثل: كهرباء، عمالة، علف).', tracking=True)
    # لم نعد نستخدم cost_type_id الذي كان يشير لـ account.analytic.tag

    pond_id = fields.Many2one('fish_farm_management.pond', string='الحوض', required=True, ondelete='restrict', tracking=True,
                              related='analytic_account_id.fish_farm_management_pond_id', store=True, readonly=True) # ربط عكسي بالحوض من الحساب التحليلي
    ...
    company_id = fields.Many2one('res.company', string='الشركة', related='analytic_account_id.company_id', store=True, readonly=True)
    ...
    employee_id = fields.Many2one('hr.employee', string='الموظف ذو الصلة', ondelete='set null')
    
    account_move_id = fields.Many2one('account.move', string='قيد المحاسبة', readonly=True, copy=False)
    ...
    def create(self, vals_list):
        ...
        records = super(PondCost, self).create(vals_list)
    ...
    def action_post_cost(self):
        for record in self:
            ...
            expense_account = self.env['account.account'].search([('code', '=', '600000'), ('company_id', '=', record.company_id.id)], limit=1)
            credit_account = self.env['account.account'].search([('code', '=', '201000'), ('company_id', '=', record.company_id.id)], limit=1)
            ...
            move_lines = [
                (0, 0, {
                    'name': _('تكلفة حوض %s: %s') % (record.pond_id.name, record.analytic_account_id.name), # استخدام اسم الحساب التحليلي
                    'account_id': expense_account.id,
                    'debit': record.amount,
                    'credit': 0,
                    'analytic_account_id': record.analytic_account_id.id, # ربط بالحساب التحليلي
                    'partner_id': record.employee_id.address_home_id.partner_id.id if record.employee_id and record.is_direct_cost else False,
                }),
                (0, 0, {
                    'name': _('دفع/استحقاق تكلفة حوض %s: %s') % (record.pond_id.name, record.analytic_account_id.name), # استخدام اسم الحساب التحليلي
