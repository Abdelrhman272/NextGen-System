...
class WaterQualityReading(models.Model):
    ...
    reading_date = fields.Datetime(string='تاريخ ووقت القراءة', required=True, default=fields.Datetime.now, tracking=True)
    
    ph = fields.Float(string='الرقم الهيدروجيني (pH)', digits=(16, 2), tracking=True)
    oxygen_level = fields.Float(string='مستوى الأكسجين المذاب (ملجم/لتر)', digits=(16, 2), tracking=True)
    temperature = fields.Float(string='درجة الحرارة (مئوية)', digits=(16, 2), tracking=True)
    ...
    salinity = fields.Float(string='الملوحة (جزء في الألف)', digits=(16, 2), tracking=True)
    
    read_by_employee_id = fields.Many2one('hr.employee', string='تمت القراءة بواسطة', ondelete='set null')
    ...
    is_alert = fields.Boolean(string='تنبيه!', compute='_compute_is_alert', store=True, help="يشير إلى ما إذا كانت أي من القراءات خارج النطاقات المحددة في الإعدادات.")
    ...
    def _compute_is_alert(self):
        ...
        for rec in self:
            ...
            if not (min_ph <= rec.ph <= max_ph):
                alert = True
                reasons.append(f"pH ({rec.ph}) خارج النطاق (%.1f-%.1f)." % (min_ph, max_ph))

            # Oxygen
            min_oxygen = float(config.get('fish_farm_management.min_oxygen', 5.0))
            if rec.oxygen_level < min_oxygen:
                alert = True
                reasons.append(f"الأكسجين ({rec.oxygen_level}) أقل من الحد الأدنى (%.1f)." % min_oxygen)
            ...
            if not (min_temp <= rec.temperature <= max_temp):
                alert = True
                reasons.append(f"درجة الحرارة ({rec.temperature}) خارج النطاق (%.1f-%.1f)." % (min_temp, max_temp))
            
            # Ammonia
            max_ammonia = float(config.get('fish_farm_management.max_ammonia', 0.05))
            if rec.ammonia > max_ammonia:
                alert = True
                reasons.append(f"الأمونيا ({rec.ammonia}) أعلى من الحد الأقصى (%.2f)." % max_ammonia)
            
            # Nitrite
            max_nitrite = float(config.get('fish_farm_management.max_nitrite', 0.1))
            if rec.nitrite > max_nitrite:
                alert = True
                reasons.append(f"النيتريت ({rec.nitrite}) أعلى من الحد الأقصى (%.2f)." % max_nitrite)
            ...
            if alert and rec.tracking_message_ids.filtered(lambda m: _('Water Quality Alert') in m.subject).filtered(lambda m: fields.Date.today() == fields.Date.context_today(m.create_date)):
                ...
                rec.message_post(
                    ...
                    body=_("تم تسجيل قراءة لجودة المياه خارج النطاق الطبيعي للحوض %s بتاريخ %s. الأسباب: \n%s") % (rec.pond_id.name, rec.reading_date, "\n".join(reasons)),
                    message_type='notification',
                    subtype_xmlid='mail.mt_note',
                    partner_ids=[(4, p.id) for p in self.env.ref('fish_farm_management.group_fish_farm_manager').users.partner_id], # إرسال للمدراء
                )