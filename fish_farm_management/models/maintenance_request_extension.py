...
class MaintenanceRequestExtension(models.Model):
    ...
    pond_id = fields.Many2one(
        ...
        store=True,
        readonly=False,
    )  # اجعله غير للقراءة ليمكن تحديده يدويا
    ...
    farm_issue_type = fields.Selection(
        ...
        string="نوع المشكلة بالمزرعة",
        help="تصنيف خاص لمشكلات الصيانة المتعلقة بعمليات المزرعة.",
    )

    # حقل لتتبع الدفعة المتأثرة إذا كانت المشكلة قد أثرت على دفعة محددة
    batch_id = fields.Many2one(
        "fish_farm_management.batch_traceability",
        string="الدفعة المتأثرة",
        help="الدفعة التي تأثرت بهذه المشكلة (مثل توقف تهوية أثر على دفعة أسماك).",
        ondelete="set null",
    )
