...
class Pond(models.Model):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', string='الحساب التحليلي للحوض', ondelete='set null',
    ...
    growth_measurement_ids = fields.One2many('fish_farm_management.fish_growth_measurement', 'pond_id', string='قياسات النمو')
    ...
    def _compute_current_fish_types(self):
        for pond in self:
            current_types = pond.fish_stocking_ids.filtered(lambda s: s.stocking_date and s.state == 'done' and not s.harvest_record_id).mapped('fish_type_id')
    ...
    def _compute_pond_summary_data(self):
        ...
        for pond in self:
            total_stocked = sum(stock.quantity for stock in pond.fish_stocking_ids.filtered(lambda s: s.state == 'done'))
            ...
            if pond.current_fish_count > 0 and pond.fish_stocking_ids:
                ...
                if active_stocking:
                    ...
                    if growth_model:
                        ...
                        estimated_biomass = (pond.current_fish_count * estimated_weight_g_per_fish) / 1000.0
                    else:
    ...
    @api.depends('water_quality_ids.ph', 'water_quality_ids.oxygen_level', 'water_quality_ids.temperature', 'water_quality_ids.reading_date')
