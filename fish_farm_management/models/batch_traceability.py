...
class BatchTraceability(models.Model):
    ...
    def _compute_batch_metrics(self):
        ...
        for batch in self:
            ...
            batch.mortality_count = sum(
                rec.mortality_count for rec in mortals
            )
            # Feed consumed
            feeds = self.env[
            ...
            batch.total_feed_consumed = sum(
                rec.quantity for rec in feeds
            )
            # Survival rate
            if batch.initial_quantity > 0:
                batch.survival_rate = (
                    batch.initial_quantity - batch.mortality_count
                ) / batch.initial_quantity * 100.0
            else:
                batch.survival_rate = 0.0
            # FCR
            if batch.final_quantity_kg > 0:
                batch.fcr = (
                    batch.total_feed_consumed / batch.final_quantity_kg
                )
            else:
                batch.fcr = 0.0
            # Estimated metrics
    ...
    def _check_dates(self):
        ...
        """
        for batch in self:
            if batch.start_date and batch.end_date and (
                batch.start_date > batch.end_date
            ):
                raise ValidationError(
                    _(
                        "تاريخ بدء الدفعة يجب أن يكون قبل تاريخ انتهائها."
                    )
                )