    ...
    def create(self, vals_list):
        ...
        for res in records:
            ...
            if res.medicine_id and res.quantity_used > 0:
                ...
                company_id = res.health_record_id.company_id.id if res.health_record_id else self.env.company.id
                
                consumption_location = res.env.ref('stock.stock_location_customers') # موقع افتراضي للاستهلاك
                picking_type = res.env['stock.picking.type'].search([('code', '=', 'internal'), ('warehouse_id.company_id', '=', company_id)], limit=1)
                if not picking_type:
                    raise UserError(_("لم يتم العثور على نوع نقل داخلي لشركتك."))

                move_vals = {
                    'name': _('علاج %s لحوض %s') % (res.medicine_id.name, res.health_record_id.pond_id.name if res.health_record_id and res.health_record_id.pond_id else ''),
                    ...
                    'state': 'draft',
                    'reference': res.health_record_id.name if res.health_record_id else '',
