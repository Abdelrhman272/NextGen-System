...
class FishHealthRecord(models.Model):
    ...
    mortality_count = fields.Integer(
        string='عدد الوفيات', default=0, tracking=True
    )
    mortality_reason = fields.Char(
        string='سبب الوفاة', tracking=True
    )
        ...
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = (
                    ...
                    .next_by_code('fish_farm_management.fish_health_record')
                    or _('New')
                )
        ...
        for rec in self:
            if rec.issue_type == 'mortality' and rec.mortality_count <= 0:
                raise ValidationError(
                    _("يجب أن يكون عدد الوفيات أكبر من صفر عند تسجيل حالة وفاة.")
                )
