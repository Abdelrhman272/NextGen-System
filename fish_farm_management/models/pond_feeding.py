...
class PondFeeding(models.Model):
    ...
    feeding_date = fields.Date(string='تاريخ التغذية/الاستخدام', required=True, default=fields.Date.context_today, tracking=True)
    
    # يمكن أن تكون منتجات علف أو أدوية
    product_id = fields.Many2one('product.product', string='المنتج (علف/دواء/مستلزم)', required=True, tracking=True,
    ...
    stock_move_id = fields.Many2one('stock.move', string='حركة المخزون', readonly=True, help='حركة المخزون التي تم إنشاؤها لاستهلاك المنتج.')
    ...
    def action_validate_feeding(self):
        for record in self:
            if record.state != 'draft':
                raise UserError(_("يمكن تأكيد سجل التغذية/المستلزمات في حالة المسودة فقط."))
            ...
            consumption_location = self.env.ref('stock.stock_location_customers') # موقع افتراضي للاستهلاك
            ...
            move_vals = {
                'name': _('استهلاك %s في حوض %s') % (record.product_id.name, record.pond_id.name),
            ...
            # ربط بالدفعات النشطة في الحوض
            active_batches = self.env['fish_farm_management.batch_traceability'].search([
                ('pond_id', '=', record.pond_id.id),
                ('end_date', '=', False) # الدفعات التي لم يتم حصادها بعد
            ])
            record.batch_ids = [(6, 0, active_batches.ids)] # ربط الدفعات المعنية
            record.message_post(body=_("تم تأكيد سجل التغذية/المستلزمات وإنشاء حركة المخزون."))

    def action_cancel_feeding(self):
        for record in self:
            if record.state == 'done':
                if record.stock_move_id and record.stock_move_id.state != 'cancel':
                    record.stock_move_id.button_cancel() # إلغاء حركة المخزون
            record.state = 'cancelled'
            record.message_post(body=_("تم إلغاء سجل التغذية/المستلزمات."))

    def unlink(self):
        for rec in self:
            if rec.state == 'done':
                raise UserError(_("لا يمكن حذف سجل تغذية/مستلزمات تم تأكيده. يرجى إلغائه أولاً."))
        return super(PondFeeding, self).unlink()