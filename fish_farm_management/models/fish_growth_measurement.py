...
class FishGrowthMeasurement(models.Model):
    ...
    measurement_date = fields.Date(string='تاريخ القياس', required=True, default=fields.Date.context_today, tracking=True)
    
    fish_type_id = fields.Many2one('product.product', string='نوع السمك المقاس', required=True, domain="[('is_fish_type', '=', True)]", tracking=True)
    
    sample_count = fields.Integer(string='عدد العينات', required=True, default=1, help='عدد الأسماك التي تم أخذ عينة منها.')
    total_sample_weight_g = fields.Float(string='إجمالي وزن العينات (جرام)', required=True, help='إجمالي وزن جميع الأسماك في العينة (جرام).')
    
    average_fish_weight_g = fields.Float(string='متوسط وزن السمكة (جرام)', compute='_compute_average_weight', store=True, help='متوسط وزن السمكة الواحدة في العينة (جرام).')
